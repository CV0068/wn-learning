...
def parse_args():
    ...
    parser.add_argument('--attr_size', dest='attr_size',
                        help='Attribute module output size', #属性模型输出尺寸
                        default=256, type=int)
    parser.add_argument('--rela_size', dest='rela_size',   #关系模型输出尺寸
                        help='Relation module output size',
                        default=256, type=int)
    parser.add_argument('--spat_size', dest='spat_size',
                        help='Spatial module output size',  #空间模型输出尺寸
                        default=256, type=int)
    ...
    parser.add_argument('--mGPUs', dest='mGPUs',
                        help='whether use multiple GPUs',  #是否多个GPU
                        action='store_true')
...
if __name__ == '__main__':
  ...
  if args.dataset == "vg":   #VG 数据集
      args.imdb_name = "vg_train"
      args.imdbval_name = "vg_val"
      args.set_cfgs = ['ANCHOR_SCALES', '[2, 4, 8, 16, 32]', 'MAX_NUM_GT_BOXES', '50'] #anchor
      cls_r_prob = pickle.load(open('data/graph/vg_graph_r.pkl', 'rb'))  #关系图数据集
      cls_r_prob = np.float32(cls_r_prob)
      cls_a_prob = pickle.load(open('data/graph/vg_graph_a.pkl', 'rb'))  #属性图
      cls_a_prob = np.float32(cls_a_prob)
  elif args.dataset == "ade":     #ADE 数据集
  ...
  imdb, roidb, ratio_list, ratio_index = combined_roidb(args.imdb_name)  #通过添加一堆元数据将roidb转换为可训练的roidb
